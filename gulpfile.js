...
gulp.task('htmlComplie', function (done) {
    ...
    gulp.src(devPath.html) //src 폴더 아래의 모든 html 파일을
        .pipe(fileinclude({
            prefix: '@@',
            basepath: '@file'
        }))
...
gulp.task('scssCompile', function (done) {
     ...
     gulp.src(devPath.scss) //_scss 폴더의 *.scss 파일을 
...
// 비디오
// MP4 파일 처리를 위한 작업
gulp.task('build-mp4', function(done) {
     gulp.src(devPath.videos) // MP4 파일이 있는 경로를 지정
      .pipe(plumber())
      .pipe(gulp.dest(buildPath.videos)) // 빌드된 MP4 파일을 저장할 경로를 지정
    //   .pipe(notify("MP4 files have been built successfully!"));
      done();
});
...
gulp.task('watch', function (done) {
    ...
    //src 디렉토리 안에 mp4 확장자를 가진 파일이 변경되면 build-mp4 task 실행
    gulp.watch(devPath.videos, gulp.series('build-mp4'));
    done();
});
...
gulp.task('default', gulp.series('jsCompile', 'scssCompile', 'htmlComplie', 'imgMinCompile', 'fontCompile', 'build-mp4', gulp.parallel('watch', 'server')));